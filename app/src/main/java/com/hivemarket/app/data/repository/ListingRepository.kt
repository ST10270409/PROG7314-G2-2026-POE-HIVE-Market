...
class ListingRepository @Inject constructor(
     ...
     *
     * The UI can collect this Flow to automatically update
     * when favourites are added or removed.
     */
     ...
     *
     * The Room database is updated first so the UI responds
     * immediately. The backend request is then attempted.
     */
    suspend fun toggleFavourite(
        ...
        val currentlyFavourite =
            favouriteDao.isFavouriteSync(listingIdStr)

        return if (currentlyFavourite) {

            // Remove locally first.
            favouriteDao.deleteFavouriteById(
                listingIdStr
            )

            // Then remove from the backend.
            try {
                ...
                Log.w(
                    TAG,
                    "Failed to remove favourite on backend",
                    e
                )
            }
            ...
            try {
                ...
                Log.w(
                    TAG,
                    "Failed to add favourite on backend",
                    e
                )
            }
    ...
    /**
     * Downloads the user's favourites from the backend
    ...
    suspend fun syncFavouritesFromRemote(
        ...
        try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                Log.d(
                    TAG,
                    "Synced ${remoteFavourites.size} favourites from backend"
                )
                ...
                Log.w(
                    TAG,
                    "Failed to fetch favourites: ${response.code()}"
                )
            }
            ...
            Log.w(
                TAG,
                "Failed to sync favourites from remote",
                e
            )
        }
    }
